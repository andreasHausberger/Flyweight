...
public class Network<T: Codable> {
    ...
    /// Performs network call with a type-agnostic array as a body
    private static func performNetworkCall(urlString: String, method: Method, urlParameter params: [String:String]? = nil, httpHeaders headers: [String: String]? = nil, body: [String: Any]? = nil, log: LoggingStyle) throws -> AnyPublisher<T,APIError>  {
        ...
        if let params = params {
            setURLParameters(components: &urlComponents, params: params)
        }
    ...
    private static func setURLParameters(components: inout URLComponents, params: [String: String]) {
...
extension Network {
    ...
    private static func performNetworkCall<B: Codable>(urlString: String, method: Method, urlParameter params: [String:String]? = nil, httpHeaders headers: [String: String]? = nil, body: B? = nil, log: LoggingStyle) throws -> AnyPublisher<T,APIError>  {
        guard var urlComponents = URLComponents(string: urlString) else { throw APIError.invalidURL }
        let config = getConfig()
        let session = URLSession(configuration: config)
        if let params = params {
            setURLParameters(components: &urlComponents, params: params)
        ...
        if let bodyObject = body {
            guard let bodyData = try? JSONEncoder().encode(bodyObject) else { throw APIError.decoding }
            request.httpBody = bodyData
        }
